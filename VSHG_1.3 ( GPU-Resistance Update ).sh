...
echo   "##############################"
echo ""
echo "This tool allows you to use a relatively weak passwort and still get very strong encryption key at the end "
...
echo "#-----------------------------------------------#"
echo "# You will have to generate custom salts and you#"
echo "# will have to use custom keyfiles              #"
echo "#################################################"
echo ""
sleep 3
fi
...
final2=$( echo -n "$gpuiterating" | ./etc/Argon2/argon2 $dsalt -d -t 1000 -p 16 -l 128 -m 12 -r  )
...
echo "$x debug"
 ...
 # end " using custom keyfile "  

 # not using custom keyfile 
...
# end of using random generated keyfile 
...
final2=$( echo -n "$gpuiterating" | ./etc/Argon2/argon2 $saltb64 -d -t 1000 -p 16 -l 128 -m 12 -r  )
...
sudo echo $final2 | sudo gpg --batch -q --passphrase-fd 0 -o "$filetoenc2.vshg" --s2k-mode 3 --s2k-count 65011712 --s2k-digest-algo sha512 --s2k-cipher-algo AES256 --symmetric --armor --cipher-algo AES256 -c "$filetoenc2" 
